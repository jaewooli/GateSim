...
### 🤝 실시간 협업 & 미니맵
- **실시간 WebSocket 멀티플레이** — 초대 링크(`?collab=roomId`)를 통해 즉시 다자간 실시간 협업 지원
- **마우스 커서 동기화** — 다른 사용자들의 마우스 위치 및 이름/컬러 실시간 표시
- **요소 편집 잠금 (Lock)** — 어떤 유저가 게이트를 드래그하고 있으면 점선 테두리 및 라벨로 타인의 편집 차단
- **피그마식 커서 챗 (Cursor Chat)** — `스페이스바 (Spacebar)`를 눌러 마우스 커서 위치에 바로 말풍선 입력창 활성화, 타이핑 시 실시간 협업 참여자들의 화면에 표시
- **전체 회로 미니맵** — 우하단 오버레이 조감도로 복잡한 회로도 편리하게 브라우징 및 클릭 이동
...
| ✅ 완료 | **WebSocket 협업** | 임시 룸, 원격 커서, 요소 단위 Lock, 피그마식 실시간 커서 챗 |
...
- **피그마식 커서 챗 추가 완료** — 스페이스바로 내 커서 챗을 켜고 엔터로 최종 메시지를 전송하며, 타인에게 실시간 타이핑 텍스트 및 5초 최종 메시지 버블 노출 구현 완료
